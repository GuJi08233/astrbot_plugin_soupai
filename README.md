...
问答判定和推理验证本质上都是「从固定几个选项里挑一个」，可以交给 [TypeSafe](https://docs.typesafe.ai) 的 Jev 模型——它只返回你预先定义的选项，不可能跑题，响应约 0.7 秒，并且每次判定都附带置信度。
...
**置信度不足、返回异常、请求失败或超时，都会自动回落到你配置的判断 LLM**，所以最坏情况与关闭时一致。两个门槛分开配置：问答判错只影响一次提问，可以宽松（默认 0.5）；推理验证直接决定游戏胜负，建议更严格（默认 0.7）。

配置项：

- **启用 Jev 加速判定** - 总开关，默认关闭。未填 API Key 时即使打开也不会生效
- **Jev API Key** - TypeSafe API Key
- **Jev API 地址** - 官方为 `https://api.typesafe.ai`；中转站需支持原生 `/v1/systemone` 端点
- **Jev 模型名** - 默认 `jev-latest`
- **问答判定的最低置信度** - 默认 0.5，填 0 表示始终采信
- **推理验证的最低置信度** - 默认 0.7，填 0 表示始终采信

用 `/汤配置` 可以查看当前是否生效。

> Jev 以英语为主要训练语言，官方说明中文等 CJK 语种准确率偏低。在本插件题库上的初步实测（35 条判定样本、5 道题）准确率约 94%，但样本量小，建议自行观察一段时间再决定是否调低置信度门槛。
